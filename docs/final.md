...
old_q_value + [$$\alpha \times $$ (current_reward + $$\gamma \times$$ max_q_value_for_state_x - old_q_value)]

old_q_value represents the old value our function assigned to the previous state and action

$$\alpha$$ represents the learning rate; how fast our function picks up on and incorporates what it observes

$$\gamma$$ represents the discount factor; how much importance we want to give to future rewards

max_q_value_for_state_x represents the maximum assigned value for the current state and all possible actions for that state.
